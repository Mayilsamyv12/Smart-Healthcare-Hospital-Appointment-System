...
from .models import CartItem, LabAppointment, LabTest, Medicine, Order
        ...
        if type == "medicine":
            ...
            if existing_items.exists():
                ...
                if existing_items.count() > 1:
                    print(
                        f"DEBUG: Found {existing_items.count()} duplicates, cleaning up..."
                    )
...
def checkout_payment(request):
            ...
            [
                {
                    ...
                    "price": float(
                        i.medicine.price if i.medicine else i.lab_test.price
                    ),
                    "quantity": i.quantity,
                }
                for i in cart_items
    ...
    return render(
        request, "commerce/checkout_payment.html", {"upi_id": "msmsayil20@oksbi"}
    )
...
def book_lab_test(request, lab_id):
    ...
    if request.method == "POST":
        ...
        LabAppointment.objects.create(
            user=request.user, lab_test=lab_test, date=date, time=time
        )
