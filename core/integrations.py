...
class GoogleIntegration:
    def __init__(self):
        ...
        if os.path.exists(CREDENTIALS_FILE) and GOOGLE_LIBS_AVAILABLE:
            ...
            print(
                "Google Integration: MOCK MODE (Syncing to onemeds001@gmail.com requires 'credentials.json')"
            )
            print(
                "Please upload 'credentials.json' and share Sheets with the Service Account email."
            )
    ...
    def add_order_to_sheet(self, order):
        ...
        if self.client:
            try:
                sheet = self.client.open(SHEET_NAME).get_worksheet(
                    1
                )  # Assuming sheet 2 for orders
    ...
    def create_calendar_event(self, appointment):
        ...
        if not self.service_calendar:
            print(
                f"[MOCK] Created Calendar Event: {appointment} for {appointment.user.email}"
            )
            return
